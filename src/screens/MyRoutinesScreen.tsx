...
export default function MyRoutinesScreen({ navigation }: any) {
  ...
  // Cargar rutina activa del usuario
  const loadRoutine = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        Alert.alert('Error', 'No se pudo cargar la rutina. Intenta más tarde.');
        setExercises([]);
      } else {
        console.log('[loadRoutine] Ejercicios cargados:', data?.length || 0);
        setExercises(data || []);
      }
    } catch (err) {
  ...
  const handleAddExercise = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        Alert.alert('Error', error || 'No se pudo agregar el ejercicio');
      } else if (data) {
        console.log('[handleAddExercise] Ejercicio agregado:', data);
        setExercises((prev) => [...prev, { ...data, dia_semana: diaSemana }]);
        ...
        onPress: async () => {
          try {
            ...
            if (error) {
              ...
              setExercises((prev) => prev.filter((ex) => ex.id_ejercicio !== id_ejercicio));
  ...
  const filteredExercises = exercises.filter((ex) => ex.dia_semana === dayValue);
